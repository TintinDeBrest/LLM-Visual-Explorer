...
def tr(key, **values):
    ...
    if config.INTERFACE_LANGUAGE not in TRANSLATIONS:
        raise ValueError(
            f"Unsupported interface language: {config.INTERFACE_LANGUAGE!r}"
        )

    language_translations = TRANSLATIONS[config.INTERFACE_LANGUAGE]

    if key not in language_translations:
        raise KeyError(
            f"Missing translation key {key!r} "
            f"for language {config.INTERFACE_LANGUAGE!r}"
        )
...
def format_number(value, decimals=1):
    ...
    if config.INTERFACE_LANGUAGE in {"fr", "es"}:
...
def format_percent(value, decimals=0):
    ...
    separator = "" if config.INTERFACE_LANGUAGE == "en" else " "
